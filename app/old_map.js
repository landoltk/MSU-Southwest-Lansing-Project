...
async function joinTableToBg(tableQueryUrl,bgQueryUrl,opts={}){
    ...
    const m = new Map();
    for (const r of rows) {
    const k = toLINK(r?.[csvKey]);
    if (k) m.set(k, r);
    }
    for (const f of (bg.features || [])) {
    const key = toLINK(f?.properties?.[geoKey]);
    const row = m.get(key);
    if (row) {
...
map.on('load', async () => {
    ...
    const norm = toLINK(f?.properties?.LINK);
    if (norm) f.properties.LINK = norm;
    }

    map.addSource('arcgis-layer', { type: 'geojson', data, promoteId: 'LINK' });
